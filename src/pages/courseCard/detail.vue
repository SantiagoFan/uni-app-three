    ...
    <view class="wrap__con">
      ...
      <view class="register" v-show="tabIndex == 0">
        <view class="register-list">
          <empty v-if="schemeList.length === 0" :title="'暂无课程信息'"></empty>
          <view class="register-list__cell" v-if="schemeList.length > 0">
            ...
            <view class="list"  v-for="(scheme,idx) in list"  :key="idx">
              <view
                ...
                @click="goRegister(scheme)"
              >
                <view class="date">余号：{{scheme.remain_count}}</view>
                <view class="rest_source"></view>
                <view class="price">申请听课证</view>
                <view class="arrow">
                  <text class="iconfont icon-arrowb"></text>
                </view>
              </view>
          ...
          <view class="order-wrap__info-user">
            ...
            <view class="info">
              ...
              <view class="info-cell">
                ...
                <view class="info-cell__text">{{ dateStr }} {{ selectScheme.qfsx == 1 ? '上午' : '下午' }}</view>
              </view>
            </view>
          </view>
          <view class="order-wrap__info-con">
            <view class="bt">请选择听课人</view>
...
export default {
  ...
  methods: {
    ...
    async createOrder() {
      ...
      this.flag = true
      let serisl_number_index = this.selectScheme["source_list"].findIndex((item) => {
        return item.scheme_status == 0
      })
      var params = {
        price: this.selectScheme.price,
        department_name: this.model.department_name,
        doctor_name: this.model.doctor_name,
        selectDate: this.selectDate,
        time: this.getTime(this.selectScheme),
        patient_name: this.patientInfo.name,
        scheme_id: this.selectScheme['scheme_id'],
        serisl_number: this.selectScheme["source_list"][serisl_number_index]['serisl_number'],
    ...
    getTime(selectScheme) {
      // 最早时间到最晚时间
      let list = selectScheme["source_list"];
      return list[0].time.split('--')[0]+"~"+
      list[list.length-1].time.split('--')[1]
    },
    changePatient(index) {
