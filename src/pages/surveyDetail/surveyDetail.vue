  ...
  <view class="wrap">
    <view class="wrap-head">
      <view class="wrap-head__art1">
        ...
        <view class="date">有效期：{{model.start_time}}  至  {{model.end_time}}</view>
      </view>
      <view class="wrap-head__art2">
        <view class="title" v-if="model.description">{{model.description}}</view>
...
export default {
	data() {
      ...
      value: 'orange',
      list: [],
      model: {name:"",start_time:"",end_time:"",description:"",author:""},
      flag: false
		}
  },
  ...
  methods: {
    getDetail(){
      this.$http.post(this.API.QUESTION_DETAIL,{questionnaireid:this.$Route.query.id}).then(res=>{
