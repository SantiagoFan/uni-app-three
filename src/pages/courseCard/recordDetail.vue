  ...
  <view class="wrap u-skeleton " id="wrap">
    <view :class="['wrap-status', { 'wrap-status__bg': data.status == 3 }]">
      <view class="wrap-status__info">
        ...
        <view class="title" v-if="data.status == 2">预约成功</view>
        <view class="title" v-if="data.status == 1">听课凭证-预约成功</view>
        <view class="title" v-if="data.status == 3">听课凭证-取消成功</view>
        <view class="title" v-if="data.status == 4">已听课</view>
      ...
      <view class="wrap-status__msg">
        <text v-if="data.status == 1"
          >预约听课可提前一天在"挂号记录"中取消，预约成功后不用取号，凭小程序推送的"挂号凭证"可直接听课</text
        >
        <text v-if="data.status == 2"
          >请在锁定的时间内完成确认，否则将取消号源。</text
        >
        <text v-if="data.status == 3">预约听课已取消，如需听课请重新提交</text>
      </view>
    </view>
    ...
    <view class="wrap-info">
      <view class="wrap-info__box">
        <view class="bt">听课人信息</view>
        <view class="list">
          <view class="cell u-skeleton-rect">
            <view class="cell-label">听课人</view>
            <view class="cell-con ">{{ data.patient_name }}</view>
          </view>
          <view class="cell u-skeleton-rect">
            <view class="cell-label">院内卡号</view>
          ...
          <view class="cell u-skeleton-rect">
            <view class="cell-label">听课时段</view>
