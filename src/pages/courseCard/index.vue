<template>
  <view class="wrap">
    <view class="wrap_top">
      <view class="wrap_top__date">日期:{{ selectDate }}</view>
    </view>
    <view class="wrap_con">
      <view
        ...
        >
          ...
          <view class="cell__info">
            <view class="title">
              ...
              <view class="right">
                <view class="sur" v-if="item.remain_count > 0">余号:{{ item.remain_count }}</view>
                <view class="tag" v-if="item.remain_count == 0">无名额</view>
              </view>
            </view>
  ...
  methods: {
    ...
    getDoctorList() {
      ...
      this.$http.post(this.API.DOCTOR_LIST, data).then((res) => {
        //
        this.doctorList = res.data.filter(e=>{ return e.doctor_name.indexOf('听课凭证')>-1 })
        this.postLock = false
      })
    },
    goDetail(item, date) {
